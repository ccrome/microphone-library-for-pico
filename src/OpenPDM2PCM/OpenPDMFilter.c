...
{
  return (int32_t)
    _C.lut[data[0]][0][sincn] +
    _C.lut[data[1]][1][sincn] +
    _C.lut[data[2]][2][sincn] +
    _C.lut[data[3]][3][sincn] +
    _C.lut[data[4]][4][sincn] +
    _C.lut[data[5]][5][sincn] +
    _C.lut[data[6]][6][sincn] +
    _C.lut[data[7]][7][sincn];
}
int32_t filter_table_stereo_64(uint8_t *data, uint8_t sincn)
{
  return (int32_t)
    _C.lut[data[0]][0][sincn] +
    _C.lut[data[2]][1][sincn] +
    _C.lut[data[4]][2][sincn] +
    _C.lut[data[6]][3][sincn] +
    _C.lut[data[8]][4][sincn] +
    _C.lut[data[10]][5][sincn] +
    _C.lut[data[12]][6][sincn] +
    _C.lut[data[14]][7][sincn];
}
int32_t filter_table_mono_128(uint8_t *data, uint8_t sincn)
{
  return (int32_t)
    ...
    _C.lut[data[6]][6][sincn] +
    _C.lut[data[7]][7][sincn] +
    _C.lut[data[8]][8][sincn] +
    _C.lut[data[9]][9][sincn] +
    _C.lut[data[10]][10][sincn] +
    _C.lut[data[11]][11][sincn] +
    _C.lut[data[12]][12][sincn] +
    _C.lut[data[13]][13][sincn] +
    _C.lut[data[14]][14][sincn] +
    _C.lut[data[15]][15][sincn];
}
int32_t filter_table_stereo_128(uint8_t *data, uint8_t sincn)
{
  return (int32_t)
    ...
    _C.lut[data[12]][6][sincn] +
    _C.lut[data[14]][7][sincn] +
    _C.lut[data[16]][8][sincn] +
    _C.lut[data[18]][9][sincn] +
    _C.lut[data[20]][10][sincn] +
    _C.lut[data[22]][11][sincn] +
    _C.lut[data[24]][12][sincn] +
    _C.lut[data[26]][13][sincn] +
    _C.lut[data[28]][14][sincn] +
    _C.lut[data[30]][15][sincn];
}
...
int32_t filter_table(uint8_t *data, uint8_t sincn, TPDMFilter_InitStruct *param)
{
  ...
  uint32_t *coef_p = &_C.coef[sincn][0];
  int32_t F = 0;
...
void Open_PDM_Filter_Init(TPDMFilter_InitStruct *Param)
{
  ...
  _C.sinc[0] = 0;
  _C.sinc[decimation * SINCN - 1] = 0;      
  convolve(_C.sinc1, decimation, _C.sinc1, decimation, _C.sinc2);
  convolve(_C.sinc2, decimation * 2 - 1, _C.sinc1, decimation, &_C.sinc[1]);     
  for(j = 0; j < SINCN; j++) {
    for (i = 0; i < decimation; i++) {
      _C.coef[j][i] = _C.sinc[j * decimation + i];
      sum += _C.sinc[j * decimation + i];
    }
  }

  _C.sub_const = sum >> 1;
  _C.div_const = _C.sub_const * Param->MaxVolume / 32768 / FILTER_GAIN;
  _C.div_const = (_C.div_const == 0 ? 1 : _C.div_const);
  ...
  for (s = 0; s < SINCN; s++)
  {
    uint32_t *coef_p = &_C.coef[s][0];
    for (c = 0; c < 256; c++)
      for (d = 0; d < decimation / 8; d++)
        _C.lut[c][d][s] = ((c >> 7)       ) * coef_p[d * 8    ] +
...
static int _print_pdm_filter = 0;
void Open_PDM_Filter_Print() {
    _print_pdm_filter = 1;
}
static void print_pdm_filter_params(TPDMFilter_InitStruct *Param) {
    if (_print_pdm_filter) {
        _print_pdm_filter = 0;
        commandline_print("LP_HZ: %f", Param->LP_HZ);
        commandline_print("HP_HZ: %f", Param->HP_HZ);
        commandline_print("Fs: %u", Param->Fs);
        commandline_print("In_MicChannels: %u", Param->In_MicChannels);
        commandline_print("Out_MicChannels: %u", Param->Out_MicChannels);
        commandline_print("Decimation: %u", Param->Decimation);
        commandline_print("MaxVolume: %u", Param->MaxVolume);
        commandline_print("Gain: %u", Param->Gain);
        for (int i = 0; i < SINCN; i++)
            commandline_print("  Coef[%d]: %u", i, Param->Coef[i]);
        commandline_print("FilterLen: %u", Param->FilterLen);
        commandline_print("OldOut   : %lld", Param->OldOut);
        commandline_print("OldIn    : %lld", Param->OldIn);
        commandline_print("OldZ     : %lld", Param->OldZ);
        commandline_print("LP_ALFA  : %u", Param->LP_ALFA);
        commandline_print("HP_ALFA  : %u", Param->HP_ALFA);
        commandline_print("div_cost, sub_const: (%u, %lld)", _C.div_const, _C.sub_const);
    }
}
...
void Open_PDM_Filter_64(uint8_t* data, uint16_t* dataOut, uint16_t volume, TPDMFilter_InitStruct *Param)
{
  ...
  for (i = 0, data_out_index = 0; i < Param->Fs / 1000; i++, data_out_index += channels) {
    ...
    Z = Param->Coef[1] + Z2 - _C.sub_const;
